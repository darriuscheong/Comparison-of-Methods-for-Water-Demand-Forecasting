...
out_size = 96
X_train0, Y_train0 = split_sequence_exog(df_train_norm,in_present, in_recent, in_distant,out_size)
X_test0, Y_test0 = split_sequence_exog(df_test_norm,in_present, in_recent, in_distant,out_size)

#Exogenous
X_train_exog0 = split_exog(df[:train_size])
X_test_exog0 = split_exog(df[train_size:])

#Proper Sequencing
normed = sc.fit_transform(dataset.reshape(-1,1))
df_norm = df
df_norm.loc[:,'flow'] = normed
X, Y = split_sequence_exog(df_norm,in_present, in_recent, in_distant,out_size)
train_size = 11139
X_train, Y_train = X[:train_size],Y[:train_size]
X_test, Y_test = X[train_size:],Y[train_size:]

exog = split_exog(df)
X_train_exog, X_test_exog = exog[:train_size],exog[train_size:]
...
def build_model(hp):
    ...
    time_features = LSTM(hp.Int('nodes_input',min_value = 32,max_value = 256, step=32), activation = 'relu', return_sequences=True)(time_input)
    time_features = LSTM(hp.Int('nodes_1',min_value = 32,max_value = 256, step=32), activation = 'relu',dropout=dropout)(time_features)
...
for it in range(1,11):
    #Train Model
    checkpoint_path = './checkpoints2/m_LSTMv4_'+str(it)
    ...
    start=time.time()
    model.fit([X_train,X_train_exog], Y_train, epochs = 60,
    ...
    #Save model weights
    #model.save_weights('m_LSTM_weights')
    ...
    print('m_LSTMv4_'+str(it),end=',')
    for i in score_arr:
        print(i,end=',')
    print(train_time)

    #Save excel
    excel_name = 'm_LSTMv4_'+str(it)+'.xlsx'

    df_results_test = pd.DataFrame({
                "testPredict":testPredict.tolist(),
                "testlower":lower.tolist(),
                "testupper":upper.tolist(),
                })

    df_results_train = pd.DataFrame({
                "trainPredict":trainPredict.tolist(),
                "trainlower":train_lower.tolist(),
                "trainupper":train_upper.tolist()
                })

    with pd.ExcelWriter(excel_name) as writer:
        df_results_test.to_excel(writer,index=False,sheet_name='Test')
        df_results_train.to_excel(writer,index=False,sheet_name='Train') 